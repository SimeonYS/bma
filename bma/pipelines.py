...
class BmaPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('bma.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `bma`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM bma WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO bma (date, title, link, content)"
